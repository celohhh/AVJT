window.addEventListener('load',pjeMenuExpandido)

async function pjeMenuExpandido(){

	let armazenamento = await browser.storage.local.get()

	CONFIGURACAO = armazenamento

	let anexarDocumento = selecionar('#anexarDocumento')
		...
		configuracoes.forEach(
			elemento => {
				if(configuracao[elemento.id])
					elemento.checked = configuracao[elemento.id]
			}
		)
	...
	async function salvarConfiguracoes(){
		...
		configuracoes.forEach(
			elemento => pjeMenu[elemento.id] = elemento.checked
		)
