...
async function criarMenuDaBarraDeFerramentasDoNavegador(){
	...
	criarBotaoFixo(
		'contribuir',
		'Se o ' + EXTENSAO.short_name + ' faz diferença na sua vida, clique aqui para contribuir com o seu desenvolvimento.',
		abrirPaginaContribuir
	)
	...
	function criarLinksUteis(){
		...
		criarBotaoDoMenu(
			...
			() => criarJanela(LINK.pje.legado.painel,'pjeLegadoPainel'),
			'pjeLegadoPainel'
		)
		criarBotaoDoMenu(
			'pje-instancia-2',
		...
		criarBotaoDoMenu(
			'eCarta',
			'Sistema E-Carta',
			() => criarJanela(LINK.eCarta,'eCarta')
		)
		criarBotaoDoMenu(
			'malote',
			'Malote Digital',
		...
		criarBotaoDoMenu(
			'sigeo',
			'SIGEO',
			() => criarJanela(LINK.sigeo.portal,'sigeo')
		)
		criarBotaoDoMenu(
			'tst',
			'TST - Trubunal Superior do Trabalho',
			() => criarJanela(LINK.tst.raiz,'tst')
		)
		criarBotaoDoMenu(
			'wikivt',
			'WikiVT',
		...
		criarBotaoDoMenu(
			'siscondj',
			'SISCONDJ-JT',
			() => criarJanela(LINK.siscondj.raiz,'siscondj')
		)
		criarBotaoDoMenu(
			'bb',
		...
		criarBotaoDoMenu(
			'trt',
			'TRT - Trubunal Regional do Trabalho',
			() => criarJanela(LINK.tribunal.portal,'trt')
		)
		criarBotaoDoMenu(
			'sinesp',
			'SINESP',
	...
	function assistentesEspecializados(){
		...
		function recebimentoInstanciaSuperior(){
			...
			ativar(
				'.recebimentoInstanciaSuperior',
				[
					'.pjeAoAbrirDetalhesDoProcesso .criarAreasDeRolagemDoDocumento',
				...
				() => {
					alternar.forEach(
						alternador => {
							...
							if(elemento)
								elemento.checked = assistente.checked
						}
					)
				}
			)
