...
//	criarAreasDeRolagemDoDocumento()
	...
	function corrigirRolagem(){
		if(!CONFIGURACAO?.pjeAoAbrirDetalhesDoProcesso?.corrigirRolagem)
			return
		estilizar('','#timeline{overflow:hidden};','avjt-corrigir-rolagem')
	}

	function criarAreasDeRolagemDaTimeline(){

		if(!CONFIGURACAO?.pjeAoAbrirDetalhesDoProcesso?.criarAreasDeRolagemDaTimeline)
			return
	...
	function criarAreasDeRolagemDoDocumento(){
		...
		if(!CONFIGURACAO?.pjeAoAbrirDetalhesDoProcesso?.criarAreasDeRolagemDoDocumento)
			return
		...
		function selecionarDocumento(){
			...
			if(!documento.pje && !documento.pdf)
				return ''
			return documento
		}
		...
		function irParaInicio(){
			selecionarDocumento()
			if(documento.pje)
				documento.pje.scrollTo(0,0)
			if(documento.pdf)
				documento.pdf.scrollTo(0,0)
			esforcosPoupados(1)
		}
		...
		function irParaFinal(){
			...
			if(!documento.pje)
				return
			if(documento.pje.scrollHeight < 900)
				return
	...
	function criarSeta(
		...
		if(funcao)
			elemento.addEventListener('mouseenter',funcao)
		if(parar)
			elemento.addEventListener('mouseout',parar)
...
function pjeDocumentoOtimizarRolagem(){
	...
	if(!CONFIGURACAO?.pje?.criarAreasDeRolagemDaPaginaDeDocumento)
		return
