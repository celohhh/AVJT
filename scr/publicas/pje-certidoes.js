...
//	if(!autenticado())
	...
	function 	criarBotaoAdicionar(){
		criarBotao(
			'avjt-certidao-adicionar',
			'botao-automacao',
			configuracoes,
			'Adicionar Botão de Certidão Automática',
			'',
			() => abrirPagina(caminho('navegador/pje-certidoes/pje-certidoes.htm'),'','','','')
		)
	}

	CONFIGURACAO.certidoes.forEach(
		(
			botao,
			indice
			) => {
			criarBotaoAutoCertidao(
				indice,
				botao.e,
				botao.f,
				botao.t,
	...
	function criarBotaoAutoCertidao(
		...
		function criarMenu(referencia){
			...
			botaoEditar.addEventListener(
				'click',
				() => abrirPagina(caminho('navegador/pje-certidoes/pje-certidoes.htm?editar='+posicao),'','','','')
			)
			botaoExcluir.addEventListener(
				...
				() => {
					...
					browser.storage.local.set({certidoes}).then(
