...
async function autogigsLancarAtividade(
	...
	function preencherPrazo(){
		if(obterData(prazo))
			autogigsPreencherCampo('dataPrazo',prazo)
		else
			autogigsPreencherCampo('dias',prazo)
	}


	function selecionarDestaque(){
		if(destaque)
			clicar('[formcontrolname="destaque"] input')
	}
		...
		if(salvar)
			setTimeout(
				() => {
					...
					if(comentario)
						autogigsLancarComentario(comentario)
				},
				1000
			)
	}
			...
			let observador = new MutationObserver(
				() => {
					...
					if(observado){
						...
						let opcao = [...opcoes].filter(
							opcao => opcao.innerText == conteudo
						)[0] || ''
						if(!opcao)
							return ''
						opcao.click()
...
function pjeAutogigsCriarBotoes(painel=''){
	...
	function 	criarBotaoAdicionar(){
		...
		criarBotao(
			'avjt-autogigs-adicionar',
			'botao-automacao',
			controles,
			'Adicionar Botão de AutoGIGS',
			'',
			() => abrirPagina(caminho('navegador/gigs/gigs.htm'),'','','','','pjeGigs')
		)
	}

	CONFIGURACAO.gigs.forEach(
		(
			botao,
			indice
		) => {
	...
	function criarBotaoAutogigs(
		...
		function obterTituloDoBotao(){
			...
			if(prazo)
				titulo += 'Prazo: ' + prazo  + '\n'
			if(responsavel)
				titulo += 'Responsável: ' + responsavel  + '\n'
			if(observacao)
				titulo += 'Observação: ' + observacao  + '\n'
			if(comentario)
				titulo += 'Comentário: ' + comentario  + '\n'
			return titulo
		}
		...
		function criarMenu(referencia){
			...
			botaoEditar.addEventListener(
				'click',
				() => abrirPagina(caminho('navegador/gigs/gigs.htm?editar='+posicao),'','','','','pjeGigs')
			)
			botaoExcluir.addEventListener(
				...
				() => {
					...
					browser.storage.local.set({gigs}).then(
						() => pjeAutogigsCriarBotoes(painel)
					)
					pjePainelSuperiorMenuDeContextoFechar()
				}
