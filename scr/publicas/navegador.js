...
function definirIconeDaExtensaoPeloEstado(ativada){
	...
	if(ativada)
		browser.browserAction.setIcon({path:'/imagens/icones/extensao/azul.svg'})
	else
		browser.browserAction.setIcon({path:'/imagens/icones/extensao/vermelho.svg'})
...
function obterConfiguracoesDaExtensao(){
	...
	document.querySelectorAll('configuracoes').forEach(
		configuracoes => {
			...
			configuracoes.querySelectorAll('input,textarea').forEach(
				configuracao => {
					...
					if(configuracao.type === 'textarea')
						configuracao.value = dados[chave] || ''
					if(chave === 'assinatura')
						configuracao.value = dados[chave] || configuracoesDaExtensaoAssinatura() || ''
					if(chave === 'emailDocumentos' || chave === 'whatsapp')
						configuracao.value = dados[chave] || configuracoesDaExtensaoTextoPadraoEmailDocumentos() || ''
...
function ativarConfiguracao(chave,configuracao){
	...
	if(destino.includes(configuracao))
		return true
			...
			configuracoes => {
				...
				configuracoes.querySelectorAll('input,textarea').forEach(
					configuracao => {
						...
						if(chave === 'assinatura')
							dados[chave] = configuracao.value || configuracoesDaExtensaoAssinatura() || ''
						if(chave === 'emailDocumentos' || chave === 'whatsapp')
							dados[chave] = configuracao.value || configuracoesDaExtensaoTextoPadraoEmailDocumentos() || ''
