async function sigeo(){
	...
	if(!JANELA.includes(LINK.sigeo.dominio))
		return
...
function sigeoInjetarVideoYoutube(){
	...
	if(
		JANELA.includes('comum/menu_sistema.jsf')
		||
		JANELA.includes('nomearprofissionais/nomearprofissionais')
		||
		JANELA.includes('pagamento/mantersolicitacaopagamento/manter')
	){
...
function sigeoSelecionarProfissional(){
	...
	function selecionarResultado(){
		if(!JANELA.includes('nomearprofissionais_popup_1_pesquisarprofissionais'))
			return
		clicar('[name*=profissional]')
		clicar('[title="Concluir"]')
	}
...
function sigeoCriarBotaoCertificar(){
	...
	criarBotao(
		...
		() => {
			...
			let tipo = 'Requisição Final de Honorários Periciais'
			let descricao = sigeoObterTexto('Nome do profissional')
			let certidao = pjeCertificar(
				sigeoObterTexto('Nome do profissional'),
				tipo,
				'CERTIFICO que protocolei, sob o número '+protocolo+', '+tipo+' em favor de '+descricao+'.'
			)
